...
- (void)layoutSubviews{
    ...
    self.titleLabel.center = CGPointMake(CGRectGetWidth(self.titleLabel.frame)/2 + 20, CGRectGetHeight(self.titleLabel.frame)/2 + 10);
}
...
- (void)prepareLayout{
    ...
    for (NSInteger section =0; section < sections; section++) {
        ...
        if (numberOfItems > 0) {
            ...
            if ([delegate respondsToSelector:@selector(collectionView:layout:titleForSectionAtIndex:)]){
                attr.title = [delegate collectionView:self.collectionView layout:self titleForSectionAtIndex:section];
            }
